...
async def warnUser(user: discord.User, sendingPerson: discord.User, reason:str=None):
    embed = discord.Embed(title=f"You've been warned!", color=discord.Color.red())
...
class Moderator(commands.Cog):
    ...
    async def _warn(self, ctx: commands.Context, member: discord.User, *, args):
        await warnUser(member, ctx.author, args)
    ...
    # Enables slow mode for the given minutes
    @commands.command(name='slowmode')
    async def _slowmode(self, ctx: commands.Context, amount: int=1):
        print("Slowmode")

        ctx.channel().slowmode_delay = amount

        embed = discord.Embed()
        embed.description(f"Changed slowmode to {amount} minutes")
